...
for (( i = 0; i < $num_nodes; i++ )); do
	...
	command="$binary_path --p2p 127.0.0.1:${p2p} --api 127.0.0.1:${api} --visual 127.0.0.1:${vis} --blockdb /tmp/prism-${i}-blockdb.rocksdb --blockchaindb /tmp/prism-${i}-blockchaindb.rocksdb --utxodb /tmp/prism-${i}-utxodb.rocksdb --walletdb /tmp/prism-${i}-wallet.rocksdb -vv --load-key ${i}.pkcs8 --fund-coins 50000"
...
for (( i = 0; i < $num_nodes; i++ )); do
	...
	url="localhost:${port}/transaction-generator/start?throttle=100"
	curl "$url" &> /dev/null
	if [ "$?" -ne 0 ]; then
