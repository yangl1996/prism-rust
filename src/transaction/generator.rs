...
use super::{Input, Output, PubkeyAndSignature, Transaction};
use crate::crypto;
...
pub fn random() -> Transaction {
    ...
    let signatures: Vec<PubkeyAndSignature> = vec![]; // It's hard to generate valid signatures, so just don't put signatures here
    return Transaction {
        input,
        output,
