...
// Network parameters
pub const NETWORK_CAPACITY: u32 = 5_000; // 0.5 MB/s == 4Mb/s. We will use 80% of capacity for the txs.
pub const NETWORK_DELAY: f32 = 2.0; // 2 sec delay
                                    // Design parameters
pub const NUM_VOTER_CHAINS: u16 = 1 as u16; //
pub const TX_BLOCK_SIZE_BYTES: u32 = 64_000; //64KB
pub const DIFFICULTY_CHANGE_INTERVAL: u64 = 25; // Levels after which the difficulty of the puzzle changes

// All the parameters below are function of the above parameters
pub const TX_THROUGHPUT: u32 = NETWORK_CAPACITY *4 / 5; // .4 MB == 3.2 Mb. (4/5)
...
pub const RATIO: (f32, f32, f32) = (
    1.0,
    NUM_VOTER_CHAINS as f32,
    (TX_THROUGHPUT as f32) / ((TX_BLOCK_SIZE_BYTES as f32) * CHAIN_MINING_RATE),
);
...
pub const CHAIN_MINING_RANGE: u32 =
    ((TOTAL_MINING_RANGE as f32) / (RATIO.0 + RATIO.1 + RATIO.2)) as u32;
...
// Transaction blocks
pub const TRANSACTION_MINING_RANGE: u32 =
    TOTAL_MINING_RANGE - PROPOSER_MINING_RANGE - VOTER_MINING_RANGE;
...
pub const PROPOSER_BLOCK_TX_BLOCK_REF_LIMIT: u32 =
    3 * TRANSACTION_MINING_RANGE / PROPOSER_MINING_RANGE; // Max number of tx blocks referred by a prop block.
...
lazy_static! {
    pub static ref INITIAL_DIFFICULTY: H256 = {
