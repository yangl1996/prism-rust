...
pub fn sample_transaction_content() -> tx_Content {
    ...
    let input1 = Input{hash: hashed_data[0], index: 1};
    let input2 = Input{hash: hashed_data[1], index: 2};
    let input_vec :Vec<Input> = vec![input1, input2];


    let output1 = Output{recipient: hashed_data[2], value: 10};
    let output2 = Output{recipient: hashed_data[3], value: 12};
    let output_vec :Vec<Output> = vec![output1, output2];

    let signature_vec :Vec<Signature> = vec![];

    let sample_transaction = Transaction {input: input_vec, output: output_vec, signatures: signature_vec};
    ...
    let transaction_content = tx_Content{transactions: transaction_vec};
    return transaction_content;
}
pub fn sample_transaction_content_hash_shouldbe() -> H256 {
    let transaction_content_hash: H256 = (&hex!(
            "9591ba700494f707a4dde613c278bda050fbbfa45d217a5b1e5f5d30f4cec2d3" )).into();
...
pub fn sample_transaction_block() -> Block {
    ...
    return Block {header, content: Content::Transaction(transaction_content), sortition_proof};
}
...
pub fn sample_proposer_content1() -> proposer_Content{
    ...
    let proposer_block_content = proposer_Content{transaction_block_hashes, proposer_block_hashes: vec![]};
...
pub fn sample_proposer_content1_hash_shouldbe() -> H256 {
    let transaction_content_hash: H256 = (&hex!(
            "92b44c4a1f245d603d9bf8befdd09cd0921f0cbfdfc00772c2e0073bd6145669" )).into();
    return transaction_content_hash;
}
...
pub fn sample_proposer_content2() -> proposer_Content{
    ...
    let proposer_block_content = proposer_Content{transaction_block_hashes, proposer_block_hashes};
    return proposer_block_content;
}
...
pub fn sample_proposer_block1() -> Block{
    ...
    let header = sample_header(); // The content root is incorrect
    let sortition_proof: Vec<H256> = vec![];// The sortition proof is bogus
    return Block {header, content: Content::Proposer(proposer_block_content), sortition_proof};
}
...
pub fn sample_voter_content1_hash_shouldbe() -> H256 {
    let transaction_content_hash: H256 = (&hex!(
            "102a65b93fbf3a56bc73299d0ecb81d44c442bdf975baf57ded034b809d19fd1" )).into();
    return transaction_content_hash;
}
