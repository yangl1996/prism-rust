...
impl<'a> BlockChainGraph<'a>{
    ...
    /// Add a new block to the graph. Called when a new block is heard.
...
//                self.tx_block_pool.add_node(tx_node); todo: Make this work. Lifetime issues
            ...
            Content::Proposer(c) => {
                ...
                /// Iterating through the list of transaction blocks referred in the content
                ...
                /// Iterating through the list of proposer blocks referred in the content
                for prop_block_hash in c.proposer_block_refs.iter(){
                    /// Extracting the reference of proposer block corresponding to prop_block_hash
                    let prop_node_referred :&PropNode = self.proposer_tree.get_prop_node_from_node_id(&prop_block_hash);
                    prop_node.add_referred_node(prop_node_referred);
