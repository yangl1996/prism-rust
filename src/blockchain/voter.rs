...
impl Chain{
    pub fn new(chain_number: u16, best_block: H256) -> Self {
        ...
        return Self{chain_number, best_block, best_level: 0, unvoted_proposer_blocks,
            min_level_unvoted_proposer_block: 1, max_level_unvoted_proposer_block: 0 //todo: 1,0!!
        };
    }

    pub fn add_voter_block(&mut self, block: H256, block_parent: H256, level: u32) -> NodeUpdateStatus {
    ...
    pub fn insert_first_unvoted(&mut self, level: u32, hash: H256){
        if level == self.max_level_unvoted_proposer_block + 1 {
            ...
            panic!("Vote at level {} is skipped", self.max_level_unvoted_proposer_block);
        }
        else if level < self.max_level_unvoted_proposer_block + 1{
    ...
    pub fn get_unvoted_prop_blocks(&self) -> Vec<H256>{
        return (self.min_level_unvoted_proposer_block..=self.max_level_unvoted_proposer_block).
                                map( |level| self.unvoted_proposer_blocks.get(&level).unwrap().clone()).collect();
    }
}
