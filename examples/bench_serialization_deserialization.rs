...
fn main() {
  ...
  println!("Block generation  rate {}", (REPEAT as f64)*1000000.0/time);
  ...
  let end = Instant::now();
  let time = end.duration_since(start).as_micros() as f64;
  println!("Block serialization rate rate {}", (REPEAT as f64)*1000000.0/time);
  ...
  let end = Instant::now();
  let time = end.duration_since(start).as_micros() as f64;
  println!("Block deserialization rate rate {}", (REPEAT as f64)*1000000.0/time);
}
