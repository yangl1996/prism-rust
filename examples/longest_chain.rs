...
fn main() {
    ...
    for _ in 0..config::KAPPA-1 {
        let p = proposer_block!();
        handle_block!(p);
        parent_hash = p.hash();
    }
    // update ledger
    let (add, remove) = blockchain.update_ledger().unwrap();
    assert_eq!(add, vec![]);
    assert_eq!(remove, vec![]);
    ...
    // update ledger
    let (add, remove) = blockchain.update_ledger().unwrap();
    assert_eq!(add, vec![transaction_1.hash(), transaction_2.hash()]);
    assert_eq!(remove, vec![]);
    let ledger = blockchain.proposer_transaction_in_ledger(100).unwrap();
    let ledger_proposer: Vec<H256> = ledger.into_iter().map(|x| x.0).collect();
    assert_eq!(
